    ...
    while((entry = readdir(dir)) != NULL)
    {
        if(strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
        {
            ...
            if(lstat(fullPath, &statbuf) == 0)
            {
                ...
                if(b>linii)    ///// conditie numar linii
                {
                    printf("%s\n", fullPath);
                }
...
void extract(char* path, char* section, char* line)
{
    ...
    for(int i=0; i<size_aux; i++)////// adaug in sir
    {
        read(fd,&calinutz,1);
        iesire[i]=calinutz;
    }
...
void permission(struct stat fileStat,char * perm,char *path,char * string)  /////////////////// pentru permisiuni
{
    ...
    sir[5] = (fileStat.st_mode & S_IXGRP) ? 'x' : '-';
    sir[6] = (fileStat.st_mode & S_IROTH) ? 'r' : '-';
    sir[7] = (fileStat.st_mode & S_IWOTH) ? 'w' : '-';
    sir[8] = (fileStat.st_mode & S_IXOTH) ? 'x' : '-';// pana aici
    sir[9] ='\0';
    if(strcmp(perm,sir)==0)
    {
        printf("%s/%s\n",path,string);
    }
    free(sir);
...
int cautare_perm(char *path, char *perm)////////////////////////////////// cautare cu optiuni
{
    ...
    while((entry = readdir(dir)) != NULL)
    {
        ...
        if(lstat(filePath, &statbuf) == 0 && strcmp(entry->d_name,".")!=0 && strcmp(entry->d_name,"..")!=0)
        {
            ...
            permission(statbuf,perm,path,entry->d_name);
        }
    }
    closedir(dir);
    return 0;
...
int main(int argc, char **argv)
{
    ...
    if(argc >= 2)
    {
        ...
        if(argc == 3 && strcmp(argv[1], "findall") == 0)    ////// aici am conditii
        {
            char* start = strchr(argv[2],'=')+1;// imi ia pathu
            char* end = argv[2] + strlen(argv[2]);
            char path[end-start];
